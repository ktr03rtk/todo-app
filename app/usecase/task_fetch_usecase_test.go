...
func TestTaskFetchUseCase(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			usecase := NewTaskFetchUsecase(taskRepository)

			taskRepository.EXPECT().FindByID(tt.taskID).Return(tt.expectedOutput, tt.expectedErrType).Times(1)
